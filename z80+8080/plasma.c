#include "benchcommon.h"

#define SCREEN               0x4000

static const unsigned char sinustable[0x100] = {
    
};

unsigned char blob[8] = {0,8,0x18,0x1c,0x7c,0x7e,0xf7,0xff};

static unsigned char xbuf[32];
static unsigned char ybuf[192];
static unsigned char line_map[192];
static unsigned char c1A,c1B;
static unsigned char c2A,c2B;

static void doplasma (void)
{
    unsigned char c1a,c1b;
    unsigned char c2a,c2b;
    unsigned char i, ii;
    register unsigned char* scrn = SCREEN;

    c1a = c1A;
    c1b = c1B;
    for (ii = 0; ii < 192; ++ii) {
        ybuf[ii] = (sinustable[c1a] + sinustable[c1b]);
        c1a += 4;
        c1b += 9;
    }
    c1A += 3;
    c1B -= 5;
    c2a = c2A;
    c2b = c2B;
    for (i = 0; i < 32; ++i) {
        xbuf[i] = (sinustable[c2a] + sinustable[c2b]);
        c2a += 3;
        c2b += 7;
    }
    c2A += 2;
    c2B -= 3;
    for (ii = 0; ii < 192; ++ii) {
        for (i = 0; i < 32; ++i, ++scrn) {
            *scrn = blob[(unsigned char)(xbuf[i] + ybuf[line_map[ii]]) >> 5];
        }
    }
}



int main (void)
{
    unsigned char v;
    unsigned char block;
	unsigned int  count = 20;
	for (v = 0; v < 192; ++v) {
		line_map[v] = (v & 0xc0) | ((v & 7) << 3) | ((v & 0x38) >> 3);
	}
	start();
    while (count) {
        doplasma ();
		--count;
    }
	end();
	while (1) {}
    return EXIT_SUCCESS;
}
